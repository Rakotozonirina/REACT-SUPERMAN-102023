...
return (
    <nav className='w-[100%]'>
        <div className='w-[100%] max-sm:relative h-[30dvh] max-sm:h-[10dvh] py-[4rem] flex items-center justify-around'>
            ...
            <ul className={`uppercase ${bar ? 'max-sm:fixed max-sm:top-0 max-sm:bottom-0 max-sm:right-0 max-sm:z-10 max-sm:flex-col max-sm:px-[100px] max-sm:py-[20px] max-sm:bg-[#C1C1C1] max-sm:flex' : 'max-sm:hidden'} flex gap-[1em] text-white font-sans text-xl antialiased font-semibold`}>
                ...
                <li className='max-sm:absolute max-sm:cursor-pointer max-sm:top-[4%] max-sm:right-[15%] sm:hidden' onClick={() => handleClick(false)}>
                    <div>
                        <FaX/>
                    </div>
                </li>
            ...
            <div className='sm:hidden'>
                <FaBars className='max-sm:block max-sm:cursor-pointer max-sm:text-white max-sm:scale-[1.8]' onClick={handleClick} />
            </div>
        </div>
    </nav>
)
